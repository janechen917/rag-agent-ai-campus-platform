...
- 正式域名（推荐用于演示与验收）：https://groupprojectteam11.vercel.app
- 预览域名（临时测试）：https://groupprojectteam11-nine.vercel.app

### 后端

- 服务基地址：https://groupprojectteam11back-production.up.railway.app
- API 基础前缀：https://groupprojectteam11back-production.up.railway.app/api/
- 健康检查：https://groupprojectteam11back-production.up.railway.app/api/health/

### 前后端对接建议

- 前端环境变量 VITE_API_BASE_URL 设为：https://groupprojectteam11back-production.up.railway.app
...
- **双回答模式** — 苏格拉底引导模式（通过提问引导学生逐步推理）与直接回答模式（清晰解答配合举例），用户可自由切换
- **文本对话** — 多轮上下文对话，自动保存历史记录，支持课程上下文关联
- **图片问答** — 学生可上传课件截图、代码截图等图片，AI 基于视觉分析回答问题
- **文件问答** — 支持上传 PDF/PPT/Word/TXT 文档后进行内容相关问答
- **课程推荐** — 基于对话关键词自动推荐相关课程
- **历史记录** — 右侧面板显示历史对话列表，可一键加载过往对话
- **语义搜索** — 基于 FAISS 向量数据库的知识语义检索
- **备用响应** — AI API 不可用时自动降级为预设知识库回复（覆盖 Python/Vue/Django/Git/SQL 等 10+ 技术领域），确保服务不中断
...
- **课程聊天室** — 每门课程独立聊天室，权限校验确保仅课程教师和已选课学生可参与
- **WebSocket 通信** — 基于 Django Channels，消息即时送达
- **多消息类型** — 支持文本、图片、文件、系统通知
- **私信功能** — 教师↔学生一对一私信（不支持同角色私信），通知中心显示未读数，支持标记已读
- **联系人管理** — 自动维护联系人列表与会话记录
- **在线状态** — 实时追踪与广播在线用户
...
- **开局挑战** — 学生可自定义辩题，或由系统随机生成辩题；可选关联课程上下文，AI 根据上下文生成立场对立的观点
- **回合对战** — 学生提交反驳后，系统从 4 维打分（**逻辑、证据、知识、结构**）并计算攻击力，AI 随后自动生成反击观点
- **战绩管理**
   - 最近战绩支持"删除"（实际为隐藏，不影响累计统计）
   - 上方累计统计（总场次/胜场/胜率）基于全量历史对局，不随单条隐藏而变化
- **勋章机制** — 首胜、暴击等条件触发勋章，勋章永久保留

#### 使用方法

1. **进入辩论场** — 点击导航菜单"AI 辩论场"，进入战绩首页
2. **发起对战** — 点击"新建对战"按钮
   - 选择或输入辩题（支持自由输入或系统推荐）
   - 可选关联课程，使 AI 基于课程内容生成观点
3. **进行回合** — 点击"反驳"输入框，提交你的反驳观点
   - 系统自动评估四维分数并显示本回合攻击力
   - AI 自动生成下一步观点，继续对战
4. **对战结束** — 满足胜利/失败/退出条件时自动结束
5. **查看战绩** — 在首页查看最近战绩和累计成就

#### 胜利判定 ✅（满足任一即结束为 `won`）

| 条件 | 说明 | 代码位置 |
|------|------|--------|
| **单回合攻击力 ≥ 88** | 某一回合表现卓越，一击制敌 | `views.py:1460` |
| **累计攻击力 ≥ 230** | 多回合持续输出，逐步压制 | `views.py:1460` |
| **回合数 ≥ 3 且平均攻击力 ≥ 78** | 表现稳定，连续高质量反驳 | `views.py:1460` |

#### 失败判定 ❌（满足条件即结束为 `lost`）

| 条件 | 说明 | 代码位置 |
|------|------|--------|
| **回合数 ≥ 5 且平均攻击力 < 68** | 坚持 5 回合以上但质量不足 | `views.py:1462` |

#### 主动退出

- 点击"退出本局"按钮可一键退出当前对战
- 退出后状态记为 **failed**（失败），符合失败战绩统计规则
- 退出的战绩可在"最近战绩"中选择隐藏，不影响学期累计成绩
...
| **AI 服务** | `POST /api/ai/chat/` | AI 文本对话（支持 socratic/direct 模式） |
| | `POST /api/ai/chat-with-image/` | AI 图片问答 |
| | `POST /api/ai/chat-with-file/` | AI 文件问答（PDF/PPT/Word/TXT） |
...
| | `POST /api/ai/quiz/{id}/send-reminders/` | 手动触发邮件提醒 |
| | `GET /api/ai/quiz/{id}/reminder-logs/` | 查看提醒记录 |
| **分析** | `GET /api/ai/teacher-analytics/` | 教师 Quiz 数据分析 |
| **聊天** | `GET /api/chat/messages/conversations/` | 对话列表（含未读数） |
| | `GET /api/chat/messages/conversation/` | 与指定用户的对话历史 |
| | `GET /api/chat/messages/users/` | 联系人列表 |
| | `POST /api/chat/messages/{id}/mark_read/` | 标记消息已读 |
| | `GET /api/chat/messages/course_messages/` | 课程公共消息 |
